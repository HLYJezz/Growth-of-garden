...
- **Music: `tryAutoplayMusic()` on load, falling back to `armMusicOnFirstTap()`** — owner wants it playing from the first page. Browsers refuse audible autoplay until the visitor has interacted with the site, so first-time students still need one tap; Chrome grants autoplay to sites a visitor uses regularly, so returning ones get it instantly. `musicStarted` is set **only on success** — setting it optimistically makes the click fallback a no-op and the app goes permanently silent.
- Arm on **`click`, never `pointerdown`**, and reveal the mute button on a **following task** (`setTimeout`, not rAF — rAF doesn't fire when the page isn't compositing). On a phone the first tap collapses the URL bar, changing `100dvh`; if the element under the finger shifts between touchstart and touchend the browser cancels the click. Doing anything visible during the gesture made the intro button look dead. This cost a real bug once.
- The 🔊 sound-tip story page now appears *after* music has already started; it still earns its place for readers with the device muted.
...
- The painting lives on its own layer, `#bgLayer` (z-index 0, beneath the petals at z1 and screens at z2). `setBg()` writes to that layer, not `#app`, so the art can be scaled without scaling the text over it.
- `setBgZoom(scale)` drives the push. The story zooms **1.05 → 1.295** across its 8 pages (`renderStory`), so the same gate reads as an approach instead of a stuck page; `setBg()` resets to 1 on every scene change. `transform-origin` is `50% 58%`, roughly the gate opening.
- **Never put `background-image` in a `transition`.** It's a *discrete* property, so it buys no cross-fade — and on `#bgLayer` it actively held the old scene, so walk pages kept showing the gate. Only `transform` is transitioned.
- Because `setBg()` is only called from `renderIntro()` and `renderWalk()`, the intro, consent and demo screens all inherit `bg0`; the demo deliberately keeps the story's final zoom, which reads as continuity. The result screen inherits the closing scene, so it lands on the sunset.
